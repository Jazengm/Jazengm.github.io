...
### Choose the papers shown on Home

The Home page reads the publication collection rather than maintaining a second citation list. Set `selected: true` in a publication record to include it under **Selected papers**; set it to `false` or omit it to hide that record from Home while keeping it in the complete Publications directory.

Each selected citation is generated as “Title, with coauthors. Journal or Preprint (Year).” The site owner is removed from the coauthor phrase using `site.name` and `authorNameMatches`. Edit the publication record to change the title, author list, venue, or year.
